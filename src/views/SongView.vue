...
export default {
  ...
  data() {
    return {
      ...
      comment_alert_msg: 'Please wait! We are submitting your comment.',
      comments: [],
      comment_count: 0,
      sort: '1'
    }
  ...
  methods: {
    async addComment(values, { resetForm }) {
      ...
      this.comment_alert_msg = 'Please wait! We are submitting your comment.'
    ...
    async getComments() {
      const commentSnapshots = await commentsCollection
        .where('songId', '==', this.$route.params.id)
        .get()

      this.comments = []

      commentSnapshots.forEach((doc) => [this.comments.push({ id: doc.id, ...doc.data() })])
    },
